...
const parseGitBranch = (
  output: string,
): { current: string; ahead: number; behind: number; hasUpstream: boolean } => {
  const lines = output.split('\n');
  let current = 'main';
  let ahead = 0;
  let behind = 0;
  ...
  for (const line of lines) {
    if (line.startsWith('##')) {
      ...
      // More precise regex to detect upstream tracking
      // Format: "branch...origin/branch [ahead 1, behind 2]" or "branch...origin/branch"
      ...
      if (match) {
        current = match[1];
        const remoteBranch = match[2]; // Should be like "origin/branch-name"
        const trackingInfo = match[3];

        // We have upstream tracking if there's a remote branch reference
        // and it follows the pattern "remote/branch"
        hasUpstream = !!remoteBranch && remoteBranch.includes('/');

        console.log('🔍 Branch tracking info:', {
          line: branchInfo,
          current,
          remoteBranch,
          hasUpstream,
  ...
  return { current, ahead, behind, hasUpstream };
};
...
ipcMain.handle(
  ...
  async (event, folderPath: string, branchName?: string) => {
    try {
      console.log('🚀 Pushing branch to origin with upstream tracking...');
      ...
      // Get branch name - use provided name or current branch
      ...
      console.log('📤 Pushing branch to origin:', targetBranch);

      // Check if branch exists on remote
      let branchExistsOnRemote = false;
      try {
        await execAsync(
          `git ls-remote --exit-code origin refs/heads/${targetBranch}`,
          {
            cwd: folderPath,
          },
        );
        ...
        console.log('📍 Branch does not exist on remote, will create');
      }
      ...
      if (branchExistsOnRemote) {
        ...
        // Branch doesn't exist on remote, push with upstream tracking
        await execAsync(`git push --set-upstream origin ${targetBranch}`, {
          cwd: folderPath,
        });
      }

      console.log('✅ Branch pushed to origin successfully');
      return {
        success: true,
        branchName: targetBranch,
        createdUpstream: !branchExistsOnRemote,
      };
    } catch (error: any) {
      ...
      if (error.stderr) {
        if (error.stderr.includes('failed to push some refs')) {
          throw new Error(
            'Push failed. The remote branch may have new commits. Try pulling first.',
          );
        } else if (error.stderr.includes('remote: Permission denied')) {
          throw new Error(
  ...
  async (
    ...
    try {
      ...
      if (statusOutput.trim() && !force) {
        // Check if both branches are at the same commit
        try {
          ...
          const targetHash = targetCommit.trim();

          console.log('🔍 Comparing commits:', { currentHash, targetHash });

          // If both branches point to the same commit, we can safely switch with uncommitted changes
          if (currentHash === targetHash) {
            console.log(
              '✅ Branches are at same commit, allowing switch with uncommitted changes',
            );

            // Switch to the branch (changes will come with us)
            await execAsync(`git checkout "${branchName}"`, {
              cwd: folderPath,
            });

            console.log('✅ Successfully switched to branch:', branchName);
            return { success: true, branchName, changesFollowed: true };
          }
        } catch (commitCheckError) {
          console.warn('Could not compare branch commits:', commitCheckError);
          // Fall through to show uncommitted changes dialog
        }

        // Branches have diverged or we couldn't check - show options dialog
      ...
      // Switch to the branch (no uncommitted changes or force flag)
