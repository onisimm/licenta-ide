...
ipcMain.handle('get-git-branch', async (event, folderPath: string) => {
  try {
    ...
    try {
      ...
      // No upstream tracking set up
      hasUpstream = false;
    }

    // Check if branch exists on origin (even without upstream tracking)
    let existsOnOrigin = false;
    try {
      await execAsync(
        `git ls-remote --exit-code origin refs/heads/${branchInfo.current}`,
        {
          cwd: folderPath,
        },
      );
    ...
    console.log(
      `✅ Git branch info: ${result.current} (↑${result.ahead} ↓${result.behind}, upstream: ${hasUpstream}, onOrigin: ${existsOnOrigin})`,
    );
    return result;
  } catch (error) {
